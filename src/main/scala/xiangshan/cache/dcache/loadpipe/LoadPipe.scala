...
class LoadPipe(id: Int)(implicit p: Parameters) extends DCacheModule with HasPerfEvents {
  ...
  val s1_wpu_pred_fail = s1_valid && wpu.io.resp.bits.s1_pred_fail
  val s1_wpu_pred_fail_and_real_hit = s1_wpu_pred_fail && s1_real_tag_match_way_dup_dc.orR
  ...
  io.lsu.s1_replayCarry.real_way_en := s1_real_tag_match_way_dup_dc
